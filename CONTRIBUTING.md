# Contributing

Thank you for your interest in contributing to my project.
As a contributor, here are the guidelines I would like you to follow.

## What can you do?

There are many different ways to contribute to this project, from documentation,
to bug fixing or even enhancements, just find the one that best fits with your skills.

## Issues

If you find a bug in the source code, please open an [Issue](../../issues).

Even better, you can submit a Pull Request with a fix.

## Submitting your changes

Once you have made some changes, just submit a PR, and I'll review it as soon as possible.

Please, try to test your code as much as you can. Also, make sure that your changes are passing the status checks.

## Commit Guidelines

I'm following the [conventional commits](https://www.conventionalcommits.org/en/v1.0.0/) format.

Also, I'm enforcing signed commits for everyone.
See [GitHub's guide][signed-commits] to know more about this topic if it's new to you.

[signed-commits]: https://docs.github.com/en/authentication/managing-commit-signature-verification/about-commit-signature-verification

## Documentation

I'm using [Markdown][markdown-guide] for the documentation.

[markdown-guide]: https://docs.github.com/en/get-started/writing-on-github/getting-started-with-writing-and-formatting-on-github/basic-writing-and-formatting-syntax

## Code of Conduct

Let's keep this project open and inclusive. The only rule is, please, be respectful to others.
